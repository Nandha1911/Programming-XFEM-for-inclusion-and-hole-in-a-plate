...
def Totbodyforceassm(i,globnumber,elementnoc,DOF,globnodesextra,fbloc1,Fbod1):
        ...
        if i in elementnoc:
          ...
          #  for m in range(len(map2)-1):
            for n in range(len(mapcol)):
                  fbglob1=fbloc1[(n*2):((n*2)+2)];
                  fbglob2=Fbod1[mapcol[n][0]:(mapcol[n][1]+1)];
                  fbglob=np.add(fbglob1,fbglob2);
                  Fbod1[mapcol[n][0]:(mapcol[n][1]+1)]=fbglob;
                     ...
                     # for m in range(len(map2)-1):
                      for n in range(len(mapcol)):
                            ...
                            #K=np.zeros((totDOF,totDOF));
                            fbglob1=fbloc1[(n*4):((n*4)+4)];
                            fbglob2=Fbod1[mapcol[n][0]:(mapcol[n][3]+1)];
                            fbglob=np.add(fbglob1,fbglob2);
                            Fbod1[mapcol[n][0]:(mapcol[n][3]+1)]=fbglob;
